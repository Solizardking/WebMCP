...
export class UserContainer extends DurableObject<Env> {
	...
	async killContainer(): Promise<void> {
		...
		if (active.includes(this.ctx.id.toString())) {
	...
	async container_initialize(): Promise<string> {
		...
		return this.env.ENVIRONMENT === 'dev' || this.env.ENVIRONMENT === 'test'
			? 'Connected to local container. Use pnpm local:reset for a fresh filesystem.'
