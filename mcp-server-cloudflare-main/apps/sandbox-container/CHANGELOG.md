...
- Add Docker-backed local setup, a separate bearer-protected Wrangler endpoint, and a seven-tool smoke check.
- Repair the container image build, nested file writes/listing, process timeouts, and active-container membership checks.
- Update sandbox HTTP dependencies and document local reset, authentication, and hosted deployment requirements.
