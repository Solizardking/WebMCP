...
See [local setup](CONTRIBUTING.md) for `pnpm local:setup` and `pnpm dev`. The local MCP endpoint is `http://127.0.0.1:8976/mcp`, protected by the bearer token in `.dev.vars`. Verify all seven tools with `pnpm smoke:local`.
