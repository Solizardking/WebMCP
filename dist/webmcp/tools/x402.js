export function createX402WebMCPTools(adapters) {
    const tools = [
        {
            name: "x402.get_payment_quote",
            title: "Get x402 payment quote",
            description: "Get a read-only x402 quote for a service or agent action settling on Solana. Does not authorize or send payment.",
            inputSchema: {
                type: "object",
                properties: {
            ...
            execute: async (input) => adapters.getPaymentQuote(input),
        },
        {
            name: "x402.stage_payment",
            title: "Stage x402 payment",
            description: "Stage an x402 payment quote in the visible page for user review. This does not sign or broadcast a Solana payment.",
            inputSchema: {
                type: "object",
