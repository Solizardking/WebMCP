...
export function createPumpMcpTool(options = {}) {
...
export function createPumpFunMcpTool(options = {}) {
    ...
    return {
        ...
        server_description: 'x402 agency live Pump.fun tape on https://x402.life/pump/mcp (Fly). Health, recent launches, and a pop-out tape widget. Read-only — never signs or trades.',
        server_url: url.toString(),
        ...(options.authorization ? { authorization: options.authorization } : {}),
...
export async function askOpenAI(options) {
    ...
    if (calls.some(item => item.error))
        throw new Error('Pump MCP tool execution failed');
    if (data.status !== 'completed')
        throw new Error(`OpenAI response ${data.status}`);
