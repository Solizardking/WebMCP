import type { InferenceRequest, InferenceResult, StagedAction, SwapQuote, SwapRequest, TokenCandidate, WalletContext, X402PaymentQuote, X402QuoteRequest } from "./types.js";
...
export type FactoryWhoamiIdentity = {
    sub: string;
    clientId?: string;
    scopes: string[];
    email?: string | null;
...
export interface FactoryAdapters {
    listFactoryAgents?(): FactoryAgentSummary[];
    produceFactoryAgent?(id: string): unknown | null;
    factoryBoardPayload?(): {
        agents: unknown[];
        [key: string]: unknown;
    };
    currentFactoryIdentity?(): FactoryWhoamiIdentity | null;
}
export interface ClawdAdapters extends SolanaAdapters, X402Adapters, ClawdComputeAdapters, SolGPTAdapters, FactoryAdapters {
}
