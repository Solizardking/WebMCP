...
export function buildX402McpServer(adapters) {
    const server = new McpServer({ name: "x402-life", version: "0.1.0" }, {
        capabilities: { tools: {} },
        instructions: "Quote before payment. A staged payment is not settled. Final Solana authorization must follow the host application's confirmation and signing policy.",
    });
    server.registerTool("x402_get_payment_quote", {
        ...
        return toolResult(quote, "Generated x402 payment quote.");
    });
    server.registerTool("x402_stage_payment", {
        title: "Stage x402 payment",
        description: "Stage a previously quoted x402 payment for user review. Does not sign or broadcast the Solana transaction.",
        inputSchema: z.object({ quoteId: z.string().min(1).max(128) }),
        annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
    }, async ({ quoteId }) => {
        const staged = await adapters.stagePayment({ quoteId });
        return toolResult(staged, staged.summary);
    });
    if (adapters.getReceipt) {
        server.registerTool("x402_get_receipt", {
            ...
            return toolResult({ receipt }, "Fetched x402 receipt.");
        });
    }
    return server;
}
