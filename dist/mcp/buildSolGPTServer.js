...
import { registerStonkTools } from './stonkTools.js';
import { registerFactoryCatalogTool } from './factoryTools.js';
export function buildSolGPTMcpServer(adapters) {
    const server = new McpServer({ name: "solgpt-solana-agent", version: "0.1.0" }, {
        capabilities: { tools: {} },
        instructions: "Use read tools to resolve tokens and quote before staging a trade. Never imply that a staged transaction has been signed or broadcast. User signature is required for final Solana execution.",
    });
    server.registerTool("solana_get_wallet_context", {
        ...
        return toolResult(data, "Read Solana wallet context.");
    });
    ...
    server.registerTool("solana_get_swap_quote", {
        ...
        return toolResult(quote, "Generated Solana swap quote.");
    });
    server.registerTool("solgpt_ask", {
        title: "Ask SOLGPT",
    ...
    server.registerTool("clawd_list_models", {
    ...
    registerStonkTools(server);
    registerFactoryCatalogTool(server, adapters);
    return server;
}
//# sourceMappingURL=buildSolGPTServer.js.map