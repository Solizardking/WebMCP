...
import type { ClawdAdapters, FactoryWhoamiIdentity } from "../shared/adapters.js";
...
function hasWhoamiScope(scopes: string[]): boolean {
  return scopes.includes("tool:whoami");
}

export function registerFactoryCatalogTool(server: McpServer, adapters: ClawdAdapters) {
  server.registerTool(
    FACTORY_CATALOG_TOOL,
    {
      title: "List factory catalog agents",
      description:
        "List mintable trading-bot factory agents hosted on this desk. Read-only; does not execute trades or expose pump write tools.",
    ...
    async ({ id }) => {
      if (id) {
        ...
        const record = produced as { id?: string } | null;
        if (record?.id) {
  ...
  server.registerTool(
    FACTORY_WHOAMI_TOOL,
    {
      ...
      description:
        "Return the Auth0 identity on the MCP bearer token. Public catalog remains usable without a token. Scope tool:whoami is required for a populated identity.",
      inputSchema: z.object({}),
      annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    },
    async () => {
      const identity: FactoryWhoamiIdentity | null = adapters.currentFactoryIdentity?.() ?? null;
      if (!identity) {
        return toolResult(
          { authenticated: false, sub: null, scopes: [] },
          "anonymous",
        );
      }
      if (!hasWhoamiScope(identity.scopes)) {
        return {
          isError: true,
          content: [{ type: "text" as const, text: "Missing required scopes: tool:whoami" }],
