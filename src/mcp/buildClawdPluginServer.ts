...
export const CLAWD_PLUGIN_TOOLS = ['get-fee-tier', 'list-skills', 'get-skill'] as const;
...
export function buildClawdPluginMcpServer(data: ClawdReferenceData = bundledClawdReferenceData) {
  ...
  }, async () => ({ content: [{ type: 'text' as const, text: data.feeTiersMarkdown }] }));
  server.registerTool('list-skills', {
    description: 'List the bundled Clawd, Solana and Pump.fun reference guides.',
    inputSchema: z.object({}),
    annotations: CLAWD_PLUGIN_TOOL_ANNOTATIONS,
  }, async () => ({ content: [{ type: 'text' as const, text: JSON.stringify(data.registry) }] }));
  server.registerTool('get-skill', {
    ...
    content: [{ type: 'text' as const, text: guides.get(id) ?? 'Unknown guide ID' }],
