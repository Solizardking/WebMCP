    ...
    if (path !== MCP_PATH) {
        try {
            ...
            if (!['http:', 'https:'].includes(origin.protocol) ||
                (path !== REGISTER_PATH && formatChannel(origin.host) !== path)) {
                return callback(false, 403, 'Origin does not match channel');
            }
        } catch { return callback(false, 403, 'Browser Origin required'); }
    }

    // Special case for MCP path - use server token from .env
    if (path === MCP_PATH) {
        // For MCP connections, we use the token from the .env file
        if (!info.req.headers.origin && clientToken && clientToken === serverToken) {
    ...
    if (clientChannel === REGISTER_PATH) {
        ...
        ws.once('message', async (message) => {
            ...
            try {
                ...
                if (typeof host !== 'string' || formatChannel(host) !== formatChannel(origin.host)) throw new Error('Origin mismatch');
                const channelPath = formatChannel(origin.host);
                ...
                console.error(`Registered channel: ${channelPath}`);
...
const main = async () => {
    ...
    httpServer.listen(PORT, process.env.WEBMCP_BIND_HOST || HOST, () => {
